...
const CreateClassPopup = ({isOpen, setIsOpen}) => {
	...
	const handleCreateClass = async () => {
		...
		try {
			...
			const {
				data: teacherData,
				error: teacherError
			} = await supabaseClient.from('teachers').select("first_name, last_name").eq('id', (await supabaseClient.auth.getUser()).data.user.id).single();
			// console.log(uuid);
			...
			const response = await fetchTimeout(`app/api/users/new-student/route.js`, 5500, {
				method: 'POST',
				headers: {
					"student_email": studentEmails,
					"class_name": className,
					"class_code": code,
					"teacher_fname": `${teacherData.first_name}`,
					"teacher_lname": teacherData.last_name,
					"jwt": jwt
				}
			});
	...
	const handleAddStudent = async () => {
		...
		try {
			...
			const response = await fetchTimeout(`/api/users/new-student`, 5500, {
				method: 'POST',
				headers: {
			...
			if (response.ok) {
				...
				const newStudent = result.data[0];
