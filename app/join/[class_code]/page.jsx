	...
	const fetchUser = async () => {
		...
		if (user.data.user != null) {
			...
			const {
				data: classData,
				error: classError
			} = await supabaseClient.from('classes').select("name, id, teacher_id").eq('class_code', class_code);
			...
			const {
				data,
				error
			} = await supabaseClient.from('student_proxies').select('classes_left, status').eq('student_id', user.data.user.id).eq('teacher_id', classData[0].teacher_id);
			if (error) {
			...
			if (data[0].classes_left[classData[0].id] > 0) {
				const {
					data: classData2,
					error: classError2
				} = await supabaseClient.from('classes').select('meeting_link').eq('class_code', class_code);
					...
					{classDoesNotExist && (
						<Card className="lg:w-[36vw] sm:w-[60vw] w-[90vw] border-2 p-10">
							<div className="text-center">
								<h1 className="font-semibold text-md sm:text-lg text-foreground text-pretty">Class you
									are looking for does not exist. Please contact your instructor for more details</h1>
							</div>
						</Card>
					)}
					{!classDoesNotExist && (<>
							...
							<>
								{!hasEnrolled && (
									<Card className="lg:w-[36vw] sm:w-[60vw] w-[90vw] border-2">
										<div className="text-center">
											<h1 className="font-bold text-foreground text-lg sm:text-xl pt-6 text-pretty pb-6">Please
												check you email for a class invite. Return to this link after accepting
												the invite</h1>
										</div>
									</Card>
								)}
								{hasEnrolled && (
									<div className="w-full flex justify-center items-center">
										{credits === null ? (
											<Skeleton className="lg:w-[36vw] sm:w-[60vw] w-[90vw] border-2 p-6"/>
										) : credits == 0 && (
											<Card className="lg:w-[36vw] sm:w-[60vw] w-[90vw] border-2">
												<div className="text-center">
													...
													<div className="mt-6 rounded-lg bg-red-500 px-4 py-3 text-red-50">
														<div className="flex items-center">
															...
															<p className="sm:text-sm text-xs font-medium text-pretty">
																We see that you have not paid yet. Please pay to join
																the class.
															</p>
														</div>
													</div>
												...
												<div className="rounded-lg bg-white p-3 pt-0">
													...
													<div
														...
														<div className='p-1 py-2 flex flex-row justify-center'>
															...
															/>
															<label htmlFor="willPay"
															       className="text-foreground text-sm sm:text-md items-center text-pretty">I
																will complete the payment</label>
														</div>
														{willPay && (
															<div className="flex justify-end">
																<Button
																	className="w-full sm:w-auto bg-green-700 hover:bg-green-500"
																	onClick={() => window.location.href = classLink}>Join
																	Class</Button>
															</div>
														)}
													</div>
						...
						{!isLoggedIn && (
							<>
								{!noAccount && (
									<Card className="lg:w-[36vw] sm:w-[60vw] w-[90vw] border-2">
										<div className="text-center">
											<h1 className="font-semibold text-md pb-4 lg:text-xl text-foreground pt-6 text-pretty">Please
												Login to Join your class</h1>
										</div>
										<div className="rounded-lg bg-white p-3 pt-0">
											<div className="grid gap-4">
												...
												<div
													className='sm:text-md text-sm cursor-pointer text-blue-700 underline w-fit'
													onClick={() => setNoAccount(true)}>Don&apos;t have an Account?
												</div>
											</div>
										</div>
									</Card>
								...
								{noAccount && (
									<Card className="lg:w-[36vw] sm:w-[60vw] w-[90vw] border-2 p-10">
										<div className="text-center">
											<h1 className="font-semibold text-md sm:text-lg text-foreground text-pretty">Please
												contact your instructor to gain access to the class</h1>
										</div>
									</Card>
								)}
							</>
