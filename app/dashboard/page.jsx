...
const Dashboard = ({ classInfo }) => {
	...
	const _classCard = (classInfo) => {
		...
		return (
			<div
				key={classInfo.id}
				className="bg-background rounded-lg border p-4 grid gap-2"
			>
						...
						<div>
							...
							<div className="pt-2">
								...
								<span className="font-light">
									{" "}
									{convertTo12HourFormat(classInfo.start_time)} to{" "}
									{convertTo12HourFormat(classInfo.end_time)}
								</span>
							</div>
						</div>
					</div>
				...
				>
					<DialogContent>
						<DialogHeader>
							...
							<DialogDescription>
								Copy the link below to share this class
							</DialogDescription>
						</DialogHeader>
						<div className="flex items-center space-x-2">
							<Input readOnly value={copyData?.classLink} />
									...
									{!loading && (
										<div className="flex flex-col bg-white border-2 m-2 rounded-xl w-full h-[65vh] items-center justify-center">
											...
											<div className="flex flex-row items-center text-muted-foreground">
												Would you like to add a new class?
											</div>
											<div
